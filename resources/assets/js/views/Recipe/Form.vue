<template>
	<div class="recipe__show">
		<div class="recipe__header">
			<h3>{{action}} Recipe</h3>
			<div>
				<button class="btn btn__primary" @click="save" :disbled="isProcessing">
					Save
				</button>
				<button @click="$router.back()" class="btn" :disabled="isProcessing">
					Cancel
				</button>
			</div>
		</div>
			...
			<div class="recipe__details">
				<div class="recipe__details_inner">
					<div class="form__group">
						<label>Name</label>
						<input type="text" class="form_control" v-model="form.name">
						<small class="error__control" v-if="error.name">{{error.name[0]}}</small>
					</div>
					<div class="form__group">
						<label>description</label>
						<textarea class="form_control" v-model="form.description"> </textarea> 
